...
instance Show RegionAST where
    show (R region) = init (function 0 (R region))
      where
        function level (R region) = case region of
            FromPoints list -> concat (replicate level " ") ++ "FromPoints " ++ show list ++ "\n"
            Rectangle width height -> concat (replicate level " ") ++ "Rectangle " ++ show width ++ " " ++ show height ++ "\n"
            Circle radius -> concat (replicate level " ") ++ "Circle " ++ show radius ++ "\n"
            Complement (R region) -> concat (replicate level " ") ++ "~\n" ++ function (level + 2) (R region)
            Union (R region1) (R region2) -> concat (replicate level " ") ++ "+\n" ++ function (level + 2) (R region1) ++ function (level + 2) (R region2)
            Intersection (R region1) (R region2) -> concat (replicate level " ") ++ "*\n" ++ function (level + 2) (R region1) ++ function (level + 2) (R region2)
            Transform transformation (R region) -> concat (replicate level " ") ++ show transformation ++ "\n" ++ function (level + 2) (R region)
...
-- declar niste operanzi aritmetici
    negate = complement
...
instance Functor TransformationShape where
    ...
    fmap f (Combine list) = Combine (map f list)
...
instance Functor RegionShape where
    ...
    fmap f (FromPoints list) = FromPoints list
...
-- aplic functi f pe toate tipurile a din TransformationCombiner
foldRegionAST :: RegionCombiner a -> RegionAST -> a
foldRegionAST f (R region) = f (fmap (foldRegionAST f) region)
-- aplic functi f pe toate tipurile a din RegionCombiner
...
toTransformation = foldTransformationAST combiner
  where
    ...
    combiner transformation = case transformation of
      ...
      Combine list -> S.combineTransformations list
...
{-
    *** TODO ***
    ...
    Răspuns: Evaluarea leneșă permite accesarea doar a unei componente a unei perechi fără a evalua întreaga pereche, economisind astfel timp și resurse.
...
basicEntityCount = foldRegionAST combiner
  where
    combiner :: RegionCombiner (Int, Int) -- (nr de regiuni, nr de transformati)
    combiner region = case region of
      Circle radius -> (1, 0) -- (o regiune, nicio transformare)
      Rectangle width height -> (1, 0) -- ------"----
      Complement region -> region -- nr de regiuni si de transformari care se afla in "region"
      Union region1 region2 -> (fst region1 + fst region2, snd region1 + snd region2)
      Intersection region1 region2 -> (fst region1 + fst region2, snd region1 + snd region2)
      Transform transformation region -> (fst region, snd region + basicTransformationCount transformation) 
      -- nr de regiuni se afla in region
      -- nr de transfomari sunt cele din region si cele din transformation
