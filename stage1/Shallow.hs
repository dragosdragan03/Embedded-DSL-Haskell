...
-- primeste un punct si returneaza un bool
{-
    Transformare bidimensională, reprezentată ca o funcție peste puncte.
-}
type Transformation = Point -> Point
-- primesc un punct si returnez un punct
{-
    *** TODO ***
...
inside = flip ($) -- trebuie sa inversez termenii si sa aplic regiune pe punct in final
-- aplic flip pentru a avea Regiune(punct). Folosesc $ pentru a face f(x) si nu f(g(x)) cum face (.)
-- e in paranteze pentru a trnasofmra un operator in functie
{-
    *** TODO ***
...
fromPoints = flip elem -- trebuie sa aplic elem punct lista dar mai intai primesc lista
...
rectangle width height = \(x, y) -> x <= width / 2 && y <= height / 2 && x >= - width / 2 && y >= - height / 2
...
plot width height region = init [char | y <- [-height..height], x <- [-width..width + 1], 
                            char <- if (x == width + 1) then 
                                "\n" 
                                else if (inside (fromIntegral x, fromIntegral (-y)) region) then 
                                    "*" 
                                    else "."]
...
promoteUnary = (.) -- compunere de functii compun Pointed a cu  (a -> b)

promoteBinary :: (a -> b -> c) -> Pointed a -> Pointed b -> Pointed c
promoteBinary f pointed1 pointed2 point = f (pointed1 (point)) (pointed2 (point)) --  compunere de 2 functii
...
translation tx ty = \(p1, p2) -> (p1 - tx, p2 - ty)
...
scaling factor = \(p1, p2) -> (p1 / factor, p2 / factor)
...
{-
    *** TODO ***
    ...
    > printPlot 2 2 $ applyTransformation (circle 2)    
    .....
    ..*..
    .***.
    ..*..
...
applyTransformation = flip (.) -- trebuie sa aplic regiune pe transformare si d asta fac flip
...
combineTransformations = foldl (flip (.)) id -- interez prin toate transformarile din lista 

-- f, g, h siprimesc point free x
{-
    h(x)
    g(h(x))
    f(g(h(x)))
    id = functia identitate
    la fiecare iteratie fac flip pentru a putea ajunge la forma f(g(h(x)))
-}
...
{-
    *** TODO ***
    ...
    Răspuns: evalueaza elemente pana gaseste elementul pe care il caut. o sa genereze cercuri pana gaseste punctul (i cazul de fata se opreste pentru ca ii dau pana la n)
-}
circles :: Int -> Region
circles n
    | n <= 0    = const False
